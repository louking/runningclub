...
########################################################################
    ...
    #----------------------------------------------------------------------
        ...
        int_attrs = ['max races','multiplier','max gender','max division']
        str_attrs = ['order by']
        for rowndx in range(1,nrows):
            ...
            for attr in thisrow_dict:
                if attr in int_attrs:
                    if thisrow_dict[attr]:
                        thisrow_dict[attr] = int(thisrow_dict[attr])
                    else:
                        thisrow_dict[attr] = None
                elif attr in str_attrs:
                    pass    # just let it pass through
